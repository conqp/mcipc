...
class ConditionalProxy(CommandProxy):
    ...
    def blocks(
        self, start: Vec3, end: Vec3, destination: Vec3, scan_mode: ScanMode
    ) -> ExecuteProxy:
        """Compares the blocks in two equally sized volumes."""
        return self._proxy(ExecuteProxy, "blocks", start, end, destination, scan_mode)
...
class StoreProxy(CommandProxy):
    ...
    def block(
        self, target_pos: Vec3, path: str, type: DataType, scale: float
    ) -> ExecuteProxy:
        """Saves the final command's return value
        as tag data within a block entity.
        """
        return self._proxy(ExecuteProxy, "block", target_pos, path, type, scale)
    ...
    def entity(
        self, target: str, path: str, type: DataType, scale: float
    ) -> ExecuteProxy:
...
class ExecuteProxy(CommandProxy):
    ...
    def positioned(
        self, *, pos: Optional[Vec3] = None, targets: Optional[str] = None
    ) -> ExecuteProxy:
    ...
    def rotated(
        self, *, rot: Optional[Rotation] = None, targets: Optional[str] = None
    ) -> ExecuteProxy:
