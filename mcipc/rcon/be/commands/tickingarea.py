...
class AddProxy(CommandProxy):
    ...
    def __call__(self, from_: Vec3, to: Vec3, name: Optional[str] = None) -> str:
        """Adds a ticking area based on locations."""
        return self._run(from_, to, name)

    def circle(self, center: Vec3, radius: int, name: Optional[str] = None) -> str:
...
class TickingareaProxy(CommandProxy):
    ...
    def remove(
        self, *, name: Optional[str] = None, position: Optional[Vec3] = None
    ) -> str:
