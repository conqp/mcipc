...
def get_args() -> Namespace:
    ...
    basic_stats_field.add_parser("host-port", help="returns the port of the server")
    basic_stats_field.add_parser("host-ip", help="returns the IP address of the server")
    # Full stats.
    full_stats_parser = subparsers.add_parser("full-stats", help="returns full stats")
    ...
    full_stats_field.add_parser("host-port", help="returns the port of the server")
    full_stats_field.add_parser("host-ip", help="returns the IP address of the server")
    full_stats_field.add_parser("players", help="returns a list of online players")
    # Checks stats.
    check_parser = subparsers.add_parser("check", help="checks for certain conditions")
...
def get_full_stats(client: Client, args: Namespace):  # pylint: disable=R0911
    ...
    if not args.field:
        return dumps(dict(full_stats), indent=args.indent)
